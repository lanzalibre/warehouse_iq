...
const { kpis, riskSignal, mitigators, actionHistory } = DC_MANAGER_DATA
...
// ─── Mitigator card ───────────────────────────────────────────────────────────
function MitigatorCard({ mitigator, onDrillDown }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <div className="text-sm font-bold text-slate-800 mb-3">{mitigator.title}</div>
      ...
      <p className="text-xs text-slate-400 mb-3">{mitigator.detail}</p>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {mitigators.map(m => (
            <MitigatorCard key={m.id} mitigator={m} onDrillDown={setDrillDown} />
          ))}
        </div>
      </div>
