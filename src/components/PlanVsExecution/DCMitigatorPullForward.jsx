...
const sim = DC_MANAGER_DATA.mitigatorSimResults.pullForward

export default function DCMitigatorPullForward({ onOpenSimulation }) {
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-center gap-3">
        <h2 className="text-base font-bold text-slate-800">Mitigation: Pull Inventory Forward</h2>
        <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-xs font-semibold flex items-center gap-1">
          <AlertTriangle size={11} /> Use with caution
        </span>
      </div>
      ...
      <div className="bg-slate-50 rounded-xl border border-slate-200 p-4">
        ...
        <p className="text-sm text-slate-700">Move 30h of workload for the next 24h to the staging area. Provides a short-term service lift but creates downstream supply chain risk.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 text-sm font-semibold text-orange-700 bg-orange-50 rounded-lg px-3 py-2">{sim.risk}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg p-3">
        ...
        <p className="text-xs text-amber-700">This option improves today's metrics but creates downstream exposure in 48–72h. Monitor supply chain buffer closely after execution.</p>
      </div>
